                            ...
                            if span_elements:
                                ...
                                if green_value:
                                    ...
                                    if green_value >= 70:  # Only find team names and odds if dratings confidence >= 70
                                      ...
                                      for cell in cells:
                                        if isinstance(cell.text, str) and len(cell.text) == 4:
                                          half_length = len(cell.text)
                                          first_score = cell.text.strip()[:half_length-2]
                                          second_score = cell.text.strip()[half_length-2:]
                                        elif cell.text[0]=="1" and len(cell.text) == 5:
                                          first_score = cell.text.strip()[:3]
                                          second_score = cell.text.strip()[3:]
                                          
                                        elif cell.text[2]=="1" and len(cell.text) == 5:
                                          first_score = cell.text.strip()[:2]
                                          second_score = cell.text.strip()[2:]
                                          
                                        elif cell.text[0]=="1" and len(cell.text) == 6:
                                          first_score = cell.text.strip()[:3]
                                          second_score = cell.text.strip()[3:]
                                          
                                        elif cell.text[2]=="1" and len(cell.text) == 6:
                                          first_score = cell.text.strip()[:3]
                                          second_score = cell.text.strip()[3:]
                                        ...
                                        if first_score > second_score and team_position == 0:
                                          ...
                                          print(spany_elements[team_position].text, team_position)
                                          win_or_lose = "Win"
                                        else:
                                          win_or_lose = "Loss"
...
if __name__ == "__main__":
    sport_url = "https://www.dratings.com/predictor/nba-basketball-predictions/"
